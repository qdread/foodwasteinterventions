...
fp_bea <- file.path(fp_rawdata, 'raw/BEA')

wb_use <- loadWorkbook(file.path(fp_bea, 'IOUse_Before_Redefinitions_PRO_DET.xlsx'))
...
use_old_processed <- read.csv(file.path(fp_bea, '389_Use_2007_PRO_BeforeRedef.csv') ,row.names=1,check.names=F,nrows=389)
wb_use_old <- loadWorkbook(file.path(fp_bea, 'IOUse_Before_Redefinitions_PRO_2007_Detail.xlsx'))
...
write.csv(new_nomatch, file = file.path(fp_bea, 'new_nomatch_codes.csv'), row.names = FALSE)
...
new_old_mapping <- read.csv(file.path(fp_bea, 'new_to_old_mapping.csv'), stringsAsFactors = FALSE)
...
old_new_mapping <- read.csv(file.path(fp_bea, 'old_to_new_mapping.csv'), stringsAsFactors = FALSE)
...
source(file.path(fp_github, '0_preprocessing/input_output_tables/reaggregate_mat.R'))
...
write.csv(use07_agg_disagg, file.path(fp_bea, 'use2007.csv'))
write.csv(use12_agg_disagg, file.path(fp_bea, 'use2012.csv'))
...
wb_make <- loadWorkbook(file.path(fp_bea, 'IOMake_Before_Redefinitions_DET.xlsx'))
...
# Compare row names in old one. Also try to reconstruct USEEIO's input format with old data to make sure they match.
make_old_processed <- read.csv(file.path(fp_bea, '389_Make_2007_PRO_BeforeRedef.csv'),row.names=1,check.names=F,nrows=389)
make_old_processed_allrows <- read.csv(file.path(fp_bea, '389_Make_2007_PRO_BeforeRedef.csv'),row.names=1,check.names=F)
wb_make_old <- loadWorkbook(file.path(fp_bea, 'IOMake_Before_Redefinitions_2007_Detail.xlsx'))
...
write.csv(make07_agg_disagg, file.path(fp_bea, 'make2007.csv'))
write.csv(make12_agg_disagg, file.path(fp_bea, 'make2012.csv'))
